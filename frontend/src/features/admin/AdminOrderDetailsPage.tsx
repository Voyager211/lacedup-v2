...
const AdminOrderDetailsPage = () => {
  ...
  return (
    <>
      <PageHeader
        title={`Order #${order?.orderId ?? orderId}`}
        subtitle={
          order && (
            <>
              <CalendarDays className="size-4 shrink-0" aria-hidden="true" />
              Placed on {formatDate(order.createdAt as string, 'full')}
            </>
          )
        }
        actions={
          <HeaderAction to="/admin/orders" icon={<ArrowLeft className="size-4" aria-hidden="true" />}>
            Back to Orders
          </HeaderAction>
        }
      />
      ...
      <div className="max-w-4xl">
        ...
        >
          {order && (
            <>
              {/* The title and the date are on the header bar now; the badges
                  stay here, next to the controls that change them. */}
