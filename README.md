...
| `npm run migrate:images` | moves pre-Cloudinary images across, repoints the database | — |
...
Every image the app stores — product photography, brand and category artwork, review photos,
avatars — goes through [image-storage.util.ts](backend/src/common/utils/image-storage.util.ts),
and nothing else calls `sharp` or writes a file. It has two backends behind one call:

| | When | Where |
|---|---|---|
| Disk | `CLOUDINARY_URL` unset | `backend/public/uploads/<folder>/`, served at `/uploads/...` |
| Cloudinary | `CLOUDINARY_URL` set | `<CLOUDINARY_FOLDER>/<folder>/`, absolute delivery URLs |

Local development and the test suite get disk and need no account; production sets the
variable because Render's filesystem does not survive a restart. The resize happens before
either backend rather than in Cloudinary's transformation pipeline, so the stored bytes are
identical in both modes and switching cannot quietly change how a product photo is cropped.

Deletion takes the value as the database holds it and works out where it lives —
`/uploads/...` is a file, a `res.cloudinary.com` URL is a public id to destroy. That is what
lets one database hold both while the old images are migrated. It works without a second
column for the id because uploads set the `public_id` themselves and store the untransformed
delivery URL, which makes the id recoverable from the URL; `publicIdFromUrl` is the inverse,
and there is a test pinning the round trip for every folder.
...
783 tests across the two packages, and they are the fastest way to understand either half.

```bash
cd backend  && npm test    # 294 — supertest against the Express app, mongodb-memory-server
...
list — Render's free tier has no static outbound IPs to allowlist instead. Set
`CLOUDINARY_URL` too: it is the one integration that is not optional here, because this
filesystem does not survive a restart. `JWT_SECRET`, `JWT_REFRESH_SECRET` and
`SESSION_SECRET` are generated by Render; everything else is optional and its feature stays
dormant until it is filled in.
...
2. **Finish Google sign-in.** Set `GOOGLE_CALLBACK_URL` on Render to
   ...
   console, verbatim.
3. **Move the old images across.** `cd backend && npm run migrate:images -- --dry-run` reports
   what it would upload; without the flag it uploads and repoints the database. It is safe to
   re-run — each file keeps its basename as its public id, so a second pass finds what the
   first uploaded instead of duplicating it. Run it against the same database the deploy uses.
4. **Expect a cold start.** Render's free tier spins the service down when idle, so the first
   request after a quiet spell takes the better part of a minute. Everything after it is
   normal speed.

## Known gaps

- Images uploaded before Cloudinary was switched on still hold `/uploads/...` paths and are
  served from the backend's disk. They work, and they keep working locally, but on Render
  they last only as long as the instance. `npm run migrate:images` moves them across; until
  it has been run against a database, that database has a mix.
