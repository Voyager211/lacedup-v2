...
transactionSchema.statics.findPendingByUser = function (userId: string) {
  return this.find({
    userId: userId,
...
transactionSchema.statics.getUserTransactionHistory = function (
  userId: string,
  limit = 50,
  skip = 0
) {
...
export = mongoose.model<ITransaction, TransactionModel>('Transaction', transactionSchema);
